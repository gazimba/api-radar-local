...
class ComentariosController {
    async create(request: Request, response: Response) {
        const paramsSchema = z.object({
            tipo: z.enum(["ponto-turistico", "evento"]),
    ...
    async listByLocal(request: Request, response: Response) {
        const paramsSchema = z.object({
            tipo: z.enum(["ponto-turistico", "evento"]),
            id: z.coerce.number().int().positive(),
        });
    ...
    async listAll(request: Request, response: Response) {
        ...
        if (tipo === "ponto-turistico") where.pontoTuristicoId = { not: null };
        if (tipo === "evento") where.eventoId = { not: null };
        if (reportados === "1") where.reports = { some: {} };
        if (q) {
            where.OR = [
    ...
    async reportar(request: Request, response: Response) {
        const { id } = z.object({ id: z.coerce.number().int().positive() }).parse(request.params);
        const { motivo } = z.object({ motivo: z.string().max(300).optional() }).parse(request.body);
        const userId = request.user!.id;

        const comentario = await prisma.comentario.findUnique({ where: { id } });
        if (!comentario) return response.status(404).json({ message: "Comentário não encontrado." });
        if (comentario.userId === userId) return response.status(400).json({ message: "Você não pode reportar seu próprio comentário." });

        await prisma.reportComentario.upsert({
    ...
    async dispensarReport(request: Request, response: Response) {
        const { id } = z.object({ id: z.coerce.number().int().positive() }).parse(request.params);

        await prisma.reportComentario.deleteMany({ where: { comentarioId: id } });
